                ...
                for (i = 0; i < response.length; i++) {
                    ...
                    if (employee_1 != [] && employee_1 != "" && employee_1 != null) {
                        ...
                        if (!flag) {
                            ...
                            if (!employee) return console.log({ 'message': 'Employee login sync  failed' });
                            const salt = await bcrypt.genSalt(10);
                        ...
                        if (!flag_2 && flag_1) {
                            for (n = 0; n < response.length; n++) {
                                if (ID_ == response[n].StylistID) {
                                    ...
                                    if (!update) return console.log({ 'message': 'Employee login update sync  failed' });
...
router.post('/employee/local', async (req, res) => {
    try {
        ...
        employee = new Employee(_.pick(req.body, ['fullName', 'mobileNumber', 'dob', 'countryCode', 'password', 'Usernm', 'email', 'bio', 'salon', 'gender', 'StylistID', 'profile', 'image', 'address', 'dateReg', 'created', 'UpdatedOn', 'salonID', 'branchID']));
        const salt = await bcrypt.genSalt(10);
        employee.password = await bcrypt.hash(employee.password, salt);
