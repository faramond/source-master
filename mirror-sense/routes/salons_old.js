        ...
        if (req.query.serviceType) {
            ...
            con.getConnection(function (err, connection) {
                ...
                if (req.query.serviceType == 1) {
                    ...
                    connection.query(sql, [lat, long, lat, 1], async function (err, result, fields) {
                        if (err) {
                            console.log('Bookings', err.message)
                            return res.status(400).send({ 'message': err.message });
                        ...
                        if (result != [] && result != "" && result != null && result.length != 0) {
                    ...
                    connection.query(sql_2, [lat, long, lat, 2], async function (err_2, result_2, fields) {
                        ...
                        if (result_2 != [] && result_2 != "" && result_2 != null && result_2.length != 0) {
                            for (j = 0; j < result_2.length; j++) {
                                var id_2 = result_2[j].Company_ID;
                                const salon_2 = await Salon.findOne({ salonID: id_2 }).select({ reviews: 1, avgRating: 1 });
                                if (!salon_2) {
                                    result_2 = JSON.stringify(result_2);
                                    result_2 = JSON.parse(result_2);
                                    result_2[j].avgRating = 0;
                                    result_2[j].NoOfReveiews = 0;
                                }
                                else {
                                    result_2 = JSON.stringify(result_2);
                                    result_2 = JSON.parse(result_2);
                                    result_2[j].avgRating = salon_2.avgRating;
                                    result_2[j].NoOfReveiews = salon_2.reviews.length;
                                }
                            }
                            res.send(result_2);
                        }
                    ...
                    connection.query(sql_3, [lat, long, lat, 3], async function (err_3, result_3, fields) {
                        ...
                        if (result_3 != [] && result_3 != "" && result_3 != null && result_3.length != 0) {
                            for (k = 0; k < result_3.length; k++) {
                                var id_3 = result_3[k].Company_ID;
                                const salon_3 = await Salon.findOne({ salonID: id_3 }).select({ reviews: 1, avgRating: 1 });
                                if (!salon_3) {
                                    result_3 = JSON.stringify(result_3);
                                    result_3 = JSON.parse(result_3);
                                    result_3[k].avgRating = 0;
                                    result_3[k].NoOfReveiews = 0;
                                }
                                else {
                                    result_3 = JSON.stringify(result_3);
                                    result_3 = JSON.parse(result_3);
                                    result_3[k].avgRating = salon_3.avgRating;
                                    result_3[k].NoOfReveiews = salon_3.reviews.length;
                                }
                            }
                            res.send(result_3);
                        }
...
router.post('/review/:Company_ID', async (req, res) => {
    try {
        ...
        if (!req.body.reviews[0].rating) res.status(422).send({ 'message': 'Rating is mandatory' });
            ...
            let data = await Salon.findOneAndUpdate({ branchID: req.params.Company_ID }, {
            ...
            let salonRating = await Salon.findOneAndUpdate({ branchID: req.params.Company_ID }, {
...
router.patch('/likes/:Company_ID', async (req, res) => {
    ...
    try {
        let flag = false;
        let check = await Salon.findOne({ branchID: req.params.Company_ID }).select({ likes: 1 });
        ...
        if (check.likes.length != 0) {
            for (i = 0; i < check.likes.length; i++) {
                if ((check.likes[i].customer == req.query.customer)) {
                    let salonData = await Salon.findOneAndUpdate({ branchID: req.params.Company_ID },
                        {
...
router.get('/review/:Company_ID', async (req, res) => {
    try {
        const salon = await Salon.findOne({ branchID: req.params.Company_ID }).select('reviews');
