...
router.get('/', async (req, res) => {
    try {
        ...
        const employee = await Booking.find().or([{ mirrorStar: req.query.mirrorStar, status: "requested" }])
            .select({ bookingID: 1, appointmentDate: 1, userName: 1, mobileNumber: 1, email: 1, servicesName: 1, dealName: 1, amountToPay: 1 })
...
router.get('/Completed', async (req, res) => {
    try {
        ...
        const employee = await Booking.find().or([{ servedBy: req.query.mirrorStar }])
            ...
            .select({ bookingID: 1, appointmentDate: 1, servicesName: 1, userName: 1, mobileNumber: 1, email: 1, amountToPay: 1, status: 1 })
